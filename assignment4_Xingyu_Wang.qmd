---
title: "Which Asian country has the most expensive diet over time? Is it increasingly worth what they eat?"
...
de_fq <- rd_fq %>%
  select(-c(`Domain Code`, `Domain`, 
            `Element Code`, `Element`, 
            `Item Code`, `Year Code`, `Note`)) %>%
  mutate(Year = str_sub(Year, 1, 4)) %>%  #统一年份格式
...
1. 补充中国的food price数据花了好久的时间，因为数据是以图片形式呈现的，无法进行抓取

2.世界各地的数据看不清楚模式，所以改了只研究亚洲的

3.修改了研究的变量之后，有一个prvelence的year数据是个区间。本来不想改，使用rd_fq <- rd_fq %>%
  mutate(is_range_year = str_detect(Year, "^\\d{4}-\\d{4}$"))
进行处理，但是因为后面需要join，所以只能统一一下。
