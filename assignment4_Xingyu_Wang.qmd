...
ggplot(plot_china_data, aes(x = year)) +
  geom_line(aes(y = energy_req, color = "Energy Requirement"), size = 1) +
  geom_line(aes(y = food_price * 30, color = "Food Price (scaled)"), size = 1, linetype = "dashed") +
  scale_y_continuous(
    name = "Energy Requirement (kcal/cap/day)",
    sec.axis = sec_axis(~ . / 40, name = "Food Price Index (2015 = 100)")
  ) +
...
ggplot(plot_data, aes(x = year, y = value, color = area)) +
  geom_line(size = 1) +
  labs(
    title = "Average Dietary Energy Requirement (2000–2023)",
    x = "Year",
    y = "kcal/cap/day",
    color = "Country"
  ) +
